/**
 * A one-off read of the Ontario mosques found in the OSM export and confirmed
 * NOT already in src/data/masjids.json (see import-osm-ontario.ts) — not a
 * data source for the Toronto app. This writes a plain report, never touches
 * src/data/masjids.json, and adds nothing to the app's Toronto-scoped
 * location model. Whether any of these become part of the app is a separate
 * decision, not made here; this only answers "what do these sites actually
 * publish."
 *
 ...
 * order" checks. It also follows a link when the homepage doesn't have
 * today's times, the same as the Toronto crawl in findTimes() — and the same
 * fix that made capturePage read a Masjidal iframe (proven on the DFW list's
 * McKinney/IANT cases) applies here too, whichever widget platform a given
 * site happens to run.
 *
 * This cannot run from Claude's own sandbox: outbound network there is
 * allowlisted and blocks arbitrary sites. It needs to run somewhere with real
 * internet — e.g. your own machine, or the manual GitHub Action
 * (.github/workflows/scrape-ontario-manual.yml) for when there's no computer
 * to run it from.
 *
 * Run:
 *   npx playwright install chromium      # once, if not already installed
 *   ANTHROPIC_API_KEY=... npx tsx scripts/scrape-ontario.ts
 *
 ...
 * billed per call (Sonnet, roughly one call per mosque — a few cents for 75).
 */
...
const INPUT = path.join(HERE, "ontario-mosques.json");
const OUTPUT = path.join(HERE, "ontario-mosques-report.json");
 ...
 * Ad-Din fast path here — nothing in this list is confirmed to be on that
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    console.error(
      "ANTHROPIC_API_KEY is not set. Get one at https://console.anthropic.com/settings/keys " +
        "and run again as: ANTHROPIC_API_KEY=sk-... npx tsx scripts/scrape-ontario.ts",
    );
    process.exit(1);
  }
